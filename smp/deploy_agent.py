...
from .utils import kill_process, opponent_generator
...
def pause():
    """
    Pauses execution until the spacebar is pressed.
    """
    from pynput import keyboard
    ...
    # Start the listener and block until it is stopped.
    log.info("PAUSED: Press spacebar to continue")
...
def run(ret):
    ...
    with keyboard.Listener(on_press=kill_process) as listener:
        while not stop_program:
            log.info(
                "CV SYSTEM [self.run]: Detect the Pets and Food" + " in the Shop Slots"
            )
            log.info(
                "CV SYSTEM [self.run]: Calls " + "[image_detection.find_the_animals]"
            )
            print(
                "PETS PATH:",
                os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "../pets/"
                ).replace("\\", "/"),
            )
            ...
            food_dir = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "../food/"
            ).replace("\\", "/")
            pets, _ = find_the_animals(
                pets_directory=pets_dir,
                food_directory=food_dir,
            )
            pets = remove_nothing(pets)
            log.info(
                "CV SYSTEM [self.run]: The detected Pets and Food in the Shop is : {}".format(
                    pets
                )
            )
            log.info(
                "GAME ENGINE [self.run]: Set Environment Shop = "
                + "detected Pets and Food"
            )
            env.player.shop = Shop(pets)
            if env.player.lives <= 3:
                log.info(
                    "GAME ENGINE [self.run]: Increment number of "
            ...
            log.info(
                "GAME ENGINE [self.run]: Get the best action"
            ...
            log.info(
                "GAME ENGINE [self.run]: Instruction given "
            ...
            if env._is_valid_action(action):
                ...
                if get_action_name(action) == "buy_food":
                    ...
                    log.info(
                        "GAME ENGINE [self.run]:"
                        + " Calls {}".format(get_action_name(action))
                        + " with parameters {}, {}".format(
                            s[action][1:], num_pets - num_food % 2
                        )
                    )
                    ...
                    get_action_name(action) == "buy_team_food"
                ):  # same behaviour as for buy_food for single animal
                    num_pets = 0
                    num_food = 0
                    for shop_slot in env.player.shop:
                        if shop_slot.slot_type == "pet":
            ...
            log.info(
                "GAME ENGINE [self.run]: Implements the action"
                + " in the Environment\n\n\n"
            )
            ...
            if get_action_name(action) == "end_turn":
                ...
                while not battle_finished:
                    # click event
                    log.info(
                        "TRIVIAL MOUSE ACTION [self.run]: clicking to skip the battle"
                    )
                    gui.click(
                        1780 * dimensions_scale[0], 200 * dimensions_scale[1]
                    )  # x, y
                    ...
                    if find_paw():
                        ...
                        if find_arena():
                            time_pause(0.2)
                            log.info(
                                "GAME ENGINE [self.run]: Game is over! Start new game"
                            )
                            gui.click(
                                600 * dimensions_scale[0], 400 * dimensions_scale[1]
                            )  # x, y
