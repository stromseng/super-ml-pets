...
def main():
    ...
    if ret.task == "train":
        ...
        if ret.infer_model is None:
            raise ValueError(
                "Please, provide the path to the model to use for deployment, by setting 'infer_model'."
            )
        elif not os.path.exists(ret.infer_model + ".zip"):
            raise ValueError(
                "The model chosen for deployment does not exist. Chosen model:",
                ret.infer_model,
            )

        log.info("Deployment model found")
        pause()
        run(ret)
    else:
        raise ValueError(
            "Unknown task specified. Available tasks include {'train', 'deploy'}, but used:",
            ret.task,
        )
